...
class AppStorage {
  ...
  static const advanceKey = "advance";

  static Future<void> init() async {
    await Hive.initFlutter();
    //Hive.init(baseBox);
    // register adapters
    Hive.registerAdapter<Advance>(AdvanceAdapter());
    // open boxes

    await Hive.openBox(baseBox);
    // // advance
    // await Hive.openBox<Advance>(advanceKey);

    //advance=await Hive.box(baseBox).get(advanceKey);

  }
  static depose() async {
    await Hive.box(baseBox).delete(advanceKey);

  }
  static Future<Advance?> getAdvance() async{
    await Hive.openBox(baseBox);
    return Hive.box(baseBox).get(advanceKey);
  }
  static Future<void> setAdvance(Advance? advance) async{
    Hive.box(baseBox).put(advanceKey,advance);
  }
